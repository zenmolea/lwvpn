...
static int tcp_listener(const char *address, const char *port)
{
    ...
    int *reuseAddr = &reuseAddrValue;
    if ((listen_fd = socket(res->ai_family, SOCK_STREAM, IPPROTO_TCP)) == -1 ||
    setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, (char *)reuseAddr, sizeof(int)) != 0) {
    err = errno;
...
// #endif
int v6onlyValue = 0;
int *v6only = &v6onlyValue;
#if defined(IPPROTO_IPV6) && defined(IPV6_V6ONLY)
    (void) setsockopt(listen_fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)v6only, sizeof(int));
#endif
...
PYBIND11_MODULE(lwvpn, m) {
  m.doc() = "pybind11 lwvpn plugin"; // optional module docstring
  m.def("run", &fake_main, "lwvpn");
}
